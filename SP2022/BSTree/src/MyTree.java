...
//    }    
    ...
    void inOrderRotate(Node p, ArrayList smallHeight) {
        if (p == null) return;
        inOrderRotate(p.left, smallHeight);
        if(p != root && height(p) < 3)        
        {
           smallHeight.add(p);
           return;
        }
                
        inOrderRotate(p.right, smallHeight);
    }  
    
    void preOrderRotate(Node p, ArrayList twoSon){
        if (p == null) return;
        ...
        preOrderRotate(p.left, twoSon);
        preOrderRotate(p.right, twoSon);
    }    
